#!/usr/bin/env python3
"""Generate and cache N-best hypotheses for full dev-other.

Loads Zipformer-S CR-CTC, runs on all dev-other utterances, extracts G=16
candidates via k2.Nbest.from_lattice with nbest_scale=1.0, and saves
per-hypothesis data (tokens, text, CTC log-prob, lengths) to JSONL.

Usage:
    python experiments/generate_nbest.py \
...
def alignment_log_prob(label_seq: list[int], log_probs_cpu: torch.Tensor) -> float:
    """Sum log P(label_t | t) along an alignment.

    log_probs_cpu: (T, V) on CPU. label_seq must have length T (one label
    per encoder frame, including blanks and self-loops  --  pre-CTC-collapse).
    """
...
def extract_nbest_with_scores(lattice, num_paths, nbest_scale, sp, log_probs_cpu):
    """Extract N-best hypotheses with CTC alignment log-probs and token IDs.

    Returns list of dicts with keys: text, tokens, ctc_log_prob,
    len_tokens, len_chars. Deduplicated by text; on collision the
    higher-scoring alignment is kept.

    Path scores are computed manually from log_probs_cpu rather than via
    k2's get_tot_scores: each lattice path has one arc per encoder frame
    with weight log_probs[t, label_t], so the alignment log-prob is just
    sum_t log_probs[t, label_t]. This is more reliable than k2's score-
    extraction APIs across versions and gives semantically correct CTC
    alignment scores (verified to match the greedy decode's score).
    """
    import k2
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Generate and cache N-best data for dev-other"
    )
    parser.add_argument(
        "--model-dir", type=Path,
    ...
    parser.add_argument("--nbest-scale", type=float, default=1.0,
                        help="Scale for k2.Nbest.from_lattice (lower = more diverse)")
...
def main():
    ...
    print("=" * 60)
    print(f"N-best Generation  --  dev-other G=16, nbest_scale={args.nbest_scale}")
    print("=" * 60)
    ...
    if abs(nbest_scale - 1.0) < 1e-6:
        out_path = output_dir / "nbest_dev_other_G16.jsonl"
    else:
        out_path = output_dir / f"nbest_dev_other_G16_scale{nbest_scale:.2f}.jsonl"
    ...
    with open(out_path, "w") as f:
        for utt_idx, (utt_id, feats, ref_text) in enumerate(
            tqdm(utterances, desc="Generating N-best")
        ):
            ...
            if greedy_entry is None:
                ...
                # Greedy text was sampled; the manually-computed greedy alignment
                # score is exact (the per-frame argmax IS the highest-prob
                # alignment), so use it instead of the dedup'd sampled score.
